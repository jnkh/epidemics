...
function update_ms_clustering(ms::EdgeCounts,C::Float64,N::Int,alpha,beta,dt)
    ...
    delta_mis,delta_mii = get_delta_mis_mii_gillespie_global(ms,C,N,alpha,beta)
...
function sample_posterior_k_i(C::Float64,ms::EdgeCounts,central_infected::Bool,attached_infected::Bool,alpha,beta)
    ...
    for (i,k_i) in enumerate(k_i_vec)
        y = (k_i + GraphClustering.get_infected_indicator(attached_infected) )/ k
        prob_vec[i] = prior_ki_fast(k_i,k-1,z_a,z_a_b,C)*update_fn(y,y^2,alpha,beta)
    end
    prob_vec /= sum(prob_vec)
    ki = sample(k_i_vec, Weights(prob_vec))
    return ki,(k-1)-ki
end
...
function get_delta_mis_mii_gillespie_global(ms::EdgeCounts,C::Float64,N::Int,alpha,beta)
    ...
    for central_infected in options_vec
        ...
        rate = GraphClustering.get_node_count(ms,central_infected)*transition_fn(y,y_sq,alpha,beta)
        push!(rates,rate)
