...
if generate_data
    ...
    #         pfix_arr[i,j] = pfix
            ...
            pfix_theory_arr[i,j] = sparsity_pfix_prediction(N,alpha,beta,k)
    ...
    #         @time _,_,pfixd,_ = Epidemics.get_simulation_yy_pp(gi,N,alpha,beta,num_trials_th,num_trials_sim=num_trials,use_theory=true)
    #         pfix_distr_arr[i,j] = pfixd
    ...
    #         pfix_complete_th_arr[j] =get_pfix(tr_complete)[1]
            ...
            tr = get_theory_result(N,alpha,beta,gi,num_trials_reg,in_parallel=in_parallel) 
    #         pfix_sim_arr[i,j] = get_pfix_sim(sr)[1]
            pfix_reg_arr[i,j] = get_pfix(tr)[1]
